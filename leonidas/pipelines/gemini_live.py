...
r"""Gemini Live pipeline used by the Leonidas application.
...
from leonidas import config as agent_config_module
from leonidas import prompts


# Supported models for the live api processor.
MODEL_LIVE_2_5 = capabilities.MODEL_LIVE_2_5
MODEL_LIVE_3_1 = capabilities.MODEL_LIVE_3_1
...
def resolve_live_model_profile(model_name: str) -> LiveModelProfile:
  ...
  profile = capabilities.resolve_model(model_name)
  return LiveModelProfile(
      ...
      function_call_mode=FunctionCallMode(profile.function_call_mode),
  )
...
def _sensitivity(
    value: str | None, *, start: bool
) -> genai_types.StartSensitivity | genai_types.EndSensitivity | None:
  if value is None:
    return None
  if start:
    return genai_types.StartSensitivity(f'START_SENSITIVITY_{value.upper()}')
  return genai_types.EndSensitivity(f'END_SENSITIVITY_{value.upper()}')


def _media_resolution(value: str) -> genai_types.MediaResolution:
  return genai_types.MediaResolution(f'MEDIA_RESOLUTION_{value.upper()}')


def create_live_connect_config(
    agent_config: agent_config_module.AgentConfig,
    profile: LiveModelProfile,
) -> genai_types.LiveConnectConfig:
  """Translates model-independent AgentConfig at the provider boundary."""
  automatic_detection = None
  vad = agent_config.vad
  if any(value is not None for value in dataclasses.asdict(vad).values()):
    automatic_detection = genai_types.AutomaticActivityDetection(
        start_of_speech_sensitivity=_sensitivity(
            vad.start_sensitivity, start=True
        ),
        end_of_speech_sensitivity=_sensitivity(
            vad.end_sensitivity, start=False
        ),
  ...
  if generation.thinking_level is not None:
    thinking_config = genai_types.ThinkingConfig(
        thinking_level=genai_types.ThinkingLevel(
            generation.thinking_level.upper()
        )
    )
  elif generation.thinking_budget is not None:
    thinking_config = genai_types.ThinkingConfig(
        thinking_budget=generation.thinking_budget
    )
  context_compression = None
  if generation.context_trigger_tokens is not None:
    context_compression = genai_types.ContextWindowCompressionConfig(
        trigger_tokens=generation.context_trigger_tokens,
        sliding_window=genai_types.SlidingWindow(
            target_tokens=generation.context_target_tokens
        ),
    )
  return genai_types.LiveConnectConfig(
      ...
      system_instruction=prompts.system_instruction(
          ...
          synchronous=(
              profile.function_call_mode == FunctionCallMode.SYNCHRONOUS
          ),
      ),
      output_audio_transcription={},
      realtime_input_config=realtime_input_config,
      response_modalities=['AUDIO'],
      media_resolution=_media_resolution(agent_config.media.model_resolution),
...
def create_live_commentator(
    ...
    agent_config: agent_config_module.AgentConfig | None = None,
) -> processor.Processor:
  ...
  if agent_config is None:
    agent_config = agent_config_module.AgentConfig.default().with_updates(
        {
    ...
    live_model_name = agent_config.model_id
    chattiness = agent_config.chattiness
  ...
  live_api_processor = live_model.LiveProcessor(
      ...
      realtime_config=create_live_connect_config(agent_config, profile),
