...
async def run(audio_path: Path, voice_path: Path, device: str) -> None:
  ...
  if not transcript or not response or duration < 0.25 or not completed:
    raise RuntimeError(
        'Cascade smoke failed its transcription/response/audio contract'
    )
  print(
      'cascade_ok=true '
...
def main(argv: list[str] | None = None) -> int:
  ...
  asyncio.run(run(args.audio, args.voice, args.device))
  return 0
